...
// billing_details JSON（计费 PRD 第 4 章）契约，供 billing、store、httpapi、迁移
// 等多层引用；本包保持契约叶子包纪律（仅依赖标准库或 pkg/）。
...
//
// 本 JSON 是持久化 Token 用量的唯一权威来源；旧 logs.prompt_tokens /
// logs.completion_tokens 聚合列在历史回填完成后删除，聚合总量一律按
// billing_details_totals.go 的唯一汇总公式从本 JSON 还原。
...
func validateBillingDetailsPayload(payload *BillingDetailsPayload) error {
	...
	tiered, err := checkedTokenSum(payload.Tokens.Cache.WriteCache5m, payload.Tokens.Cache.WriteCache1h)
	if err != nil {
		return err
	}
