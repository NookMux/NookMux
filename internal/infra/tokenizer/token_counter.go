...
func getImageToken(c *gin.Context, fileMeta *shared.FileMeta, model string, stream bool) (int, error) {
	...
	// 使用统一的文件服务获取图片配置
	config, format, err := media.GetImageConfig(c, fileMeta.Source)
	if err != nil {
		return 0, err
	}
...
func EstimateRequestToken(c *gin.Context, meta *shared.TokenCountMeta, info *relaycommon.RelayInfo) (int, error) {
	...
	for _, file := range meta.Files {
		...
		if file.FileType == "" || (file.Source.IsURL() && shouldFetchFiles) {
			...
			cachedData, err := media.LoadFileSource(c, file.Source, "token_counter")
			if err != nil {
				if shouldFetchFiles {
			...
			file.FileType = media.DetectFileType(cachedData.MimeType)
		}
	}
...
func CountAudioTokenInput(audioBase64 string, audioFormat string) (int, error) {
	...
	duration, err := media.ParseAudio(audioBase64, audioFormat)
	if err != nil {
		return 0, err
	}
