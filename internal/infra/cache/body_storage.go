...
// ErrRequestBodyTooLarge 请求体超过大小上限的错误（内存/磁盘存储与 HTTP
// 边界共用；httpapi 侧的 GetRequestBody 会对该错误再包装具体上限值）。
...
func CreateBodyStorage(data []byte) (BodyStorage, error) {
	...
	if IsDiskCacheEnabled() &&
		...
		if err != nil {
			// 如果磁盘存储失败，回退到内存存储
			common.SysError(fmt.Sprintf("failed to create disk storage, falling back to memory: %v", err))
