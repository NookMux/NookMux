...
| 支付跳转url | payurl | String | $BASE_URL/pay/wxpay/202010903/ | 如果返回该字段，则直接跳转到该url支付 |
...
| --- | --- | --- | --- | --- | --- |
| 商户ID | pid | 是 | Int | 1001 |  |
| 易支付订单号 | trade_no | 是 | String | 20160806151343349021 | 昔年网络订单号 |
...
URL地址：$BASE_URL/api.php?act=query&pid={商户ID}&key={商户密钥}
...
URL地址：$BASE_URL/api.php?act=settle&pid={商户ID}&key={商户密钥}
...
URL地址：$BASE_URL/api.php?act=order&pid={商户ID}&key={商户密钥}&out_trade_no={商户订单号}
...
| 易支付订单号 | trade_no | String | 2016080622555342651 | 昔年网络订单号 |
...
URL地址：$BASE_URL/api.php?act=orders&pid={商户ID}&key={商户密钥}
...
URL地址：$BASE_URL/api.php?act=refund
