...
3. **`logs.id` 升为 `bigint`**：`logs` 是写入量最大的表，`int` 自增 ID 上限约 21.4 亿，按每日 1000 万条约 7 个月耗尽。同项目 `audit_logs`、`dynamic_ratio_rules`、`minimax_voices` 的主键均已是 `int64`。MySQL 存量大表需评估 ALTER 锁表成本。
